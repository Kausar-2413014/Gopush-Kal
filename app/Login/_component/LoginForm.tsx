...
export default function LoginForm({ onSwitchMode }: LoginFormProps) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (storedUsers) {
      ...
      // Cari apakah ada data yang username dan password-nya cocok
      const foundUser = users.find((u: any) => u.username === formData.username && u.password === formData.password);

      if (foundUser) {
        // Jika ketemu, set user ini sebagai user yang aktif
        localStorage.setItem("gopushkal_currentUser", foundUser.username);
        router.push("/Dashboard");
      } else {
        setErrorMsg("Username atau password salah.");
      }
    } else {
      setErrorMsg("Belum ada akun yang terdaftar. Silakan Sign Up terlebih dahulu.");
    }
  };
